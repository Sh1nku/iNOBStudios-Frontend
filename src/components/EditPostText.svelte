    ...
    function saveData() {
        ...
        fetch($session.api_url + '/Post/PostVersion/', {
            ...
            body: JSON.stringify({
                ...
                rawText: postVersion.previewText + '\n' + postVersion.rawText,
                previewText: postVersion.previewText
            }),
            method: 'PUT'
        }).then((response) => {
...
<div>
    {#if postVersion}
        <div>
            <textarea
                bind:value={postVersion.previewText}
                on:input={() => {
                    updateData();
                }}
            ...
            />
            <textarea
                bind:value={postVersion.rawText}
                on:input={() => {
                    updateData();
                }}
                ...
                <table>
                    <tbody>
                        {#each references as reference}
                            <tr>
                                <td valign="top"
                                    ><a href={'#ref_' + reference.count + '_top'}
                                        ><strong
                                            >[{reference.count}
                                            ]</strong
                                        ></a
